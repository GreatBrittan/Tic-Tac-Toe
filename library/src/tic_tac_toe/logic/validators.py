...
def validate_grid(grid: Grid) -> None:
    if not re.match(r"^[\sXO]{9}$", grid.cells):
        raise ValueError("Must contain 9 cells of: X, O, or WhiteSpace")
...
def validate_number_of_marks(grid: Grid) -> None:
    if abs(grid.x_count - grid.o_count) > 1:
        raise InvalidGameState("Invalid number of Xs and Os")

def validate_starting_mark(grid: Grid, starting_mark: Mark) -> None:
    if grid.x_count > grid.o_count:
        if starting_mark != "X":
            raise InvalidGameState("Invalid starting mark")
...
def validate_winner(
    ...
    if winner == "X":
        if starting_mark == "X":
            if grid.x_count <= grid.o_count:
                raise InvalidGameState("Invalid number of Xs")
        else:
            if grid.x_count != grid.o_count:
                raise InvalidGameState("Invalid number of Xs")
    elif winner == "O":
        if starting_mark == "O":
            if grid.o_count <= grid.x_count:
                raise InvalidGameState("Invalid number of Os")
        else:
            if grid.o_count != grid.x_count:
                raise InvalidGameState("Invalid number of Os")
