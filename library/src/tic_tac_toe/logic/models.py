...
class Mark(enum.StrEnum):
    ...
    # Assign the other player the mark not chosen
    @property
    def other(self) -> Mark:
...
class Grid:
    ...
    # Checks the input cells are comprised of all Xs Os or spaces before instantiating
    def __post__init__(self) -> None:
        validate_grid(self)
    
    # Counts the amount of X's
    @cached_property
    def x_count(self) -> int:
        return self.cells.count("X")
    
    # Counts the amount of O's
    @cached_property
...
# Create a dataclass to hold data on each move
@dataclass(frozen=True)
class Move:
    mark: Mark
    cell_index: int
